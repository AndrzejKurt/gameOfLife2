...
class GameBuilder {
	...
	createBoard() {
		...
		const cellHeight = Math.floor(
			this.gameWrapper.clientHeight / this.size.height
		)
		...
		this.gameTableCurrent.forEach((rowItems, rowIndex) => {
			...
			rowItems.forEach((cellValue, cellIndex) => {
				...
				cell.addEventListener('click', this.onClickCellHandler.bind(this))
			})
		...
		while (this.startGame === true) {
			...
			this.gameTableCurrent.forEach((rowItem, rowIndex) => {
				rowItem.forEach((cellItem, cellIndex) => {
					...
					this.gameTableCurrent[cellY][cellX] === 0
						? cell.classList.remove('active')
						: cell.classList.add('active')
					const neighborhoodItems = this.getNeighborhood(cellY, cellX)
					const activeNeighborhoods = neighborhoodItems.filter(
						address => this.gameTableCurrent[address[0]][address[1]] !== 0
					)
					...
					if (this.gameTableCurrent[cellY][cellX] === 1) {
						if (activeNeighborhoods.length < 2) {
							...
							activeNeighborhoods.length === 2 ||
							activeNeighborhoods.length === 3
						) {
	...
	getNeighborhood(y, x) {
		...
		const topRight =
			y === 0 || x === this.size.height - 1 ? null : [y - 1, x + 1]
		const right = x === this.size.width - 1 ? null : [y, x + 1]
		const left = x === 0 ? null : [y, x - 1]
		const bottomLeft =
			y === this.size.height - 1 || x === 0 ? null : [y + 1, x - 1]
		const bottom = y === this.size.height - 1 ? null : [y + 1, x]
		const bottomRight =
			y === this.size.height - 1 || x === this.size.width - 1
				? null
				: [y + 1, x + 1]
		return [
			topLeft,
